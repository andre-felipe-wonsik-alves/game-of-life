...
layout (local_size_x = 32, local_size_y = 32, local_size_z = 1) in; // criando threads
...
int getLiveNeighbours(int x, int y){
    ...
    for(int i = -1; i <= 1; i++){
        for(int j = -1; i <= 1; j++){
            if(i == 0 && j == 0) continue; // não incluir o próprio pixel na vizinhança
